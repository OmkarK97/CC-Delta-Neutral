...
contract RiveraAutoCompoundingVaultV2PrivateTest is Test  {
    ...
    function setUp() public {
        ...
        RiveraAutoCompoundingVaultV2Private myVault = new RiveraAutoCompoundingVaultV2Private(_underlying_, "MockERC4626", "MockERC4626", 172800, type(uint256).max);
    ...
    function setUpVault(address user, uint depositedAssets, uint256 userAssets, int yield_) public virtual {
        ...
        Ownable(_vault_).transferOwnership(user);
    ...
    //Tests the property that any user can deposit
    function test_deposit_not_owner(address user, uint depositedAssets, uint256 userAssets, int yield) public virtual {
        setUpVault(user, depositedAssets, userAssets, yield);
        vm.prank(user); Ownable(_vault_).transferOwnership(address(this));
        userAssets = bound(userAssets, 0, IERC20(_underlying_).balanceOf(user));
        vm.prank(user); IMockERC20(_underlying_).approve(_vault_, userAssets);
    ...
    //Tests the property that any user can mint
    function test_mint_not_owner(address user, uint depositedAssets, uint256 userAssets, int yield, uint256 shares) public virtual {
        setUpVault(user, depositedAssets, userAssets, yield);
        vm.prank(user); Ownable(_vault_).transferOwnership(address(this));
        shares = bound(shares, 0, IERC4626(_vault_).convertToShares(IERC20(_underlying_).balanceOf(user)));
        vm.prank(user); IMockERC20(_underlying_).approve(_vault_, userAssets);
    ...
    function test_withdraw_not_owner(address user, uint depositedAssets, uint256 userAssets, int yield) public virtual {
        setUpVault(user, depositedAssets, userAssets, yield);
        vm.prank(user); Ownable(_vault_).transferOwnership(address(this));
        depositedAssets = bound(depositedAssets, 0, IERC4626(_vault_).convertToAssets(IERC20(_vault_).balanceOf(user)));
        vm.expectRevert("Ownable: caller is not the owner");
    ...
    function test_redeem_not_owner(address user, uint depositedAssets, uint256 userAssets, int yield, uint256 shares) public virtual {
        setUpVault(user, depositedAssets, userAssets, yield);
        vm.prank(user); Ownable(_vault_).transferOwnership(address(this));
        shares = bound(shares, 0, IERC20(_vault_).balanceOf(user));
        vm.expectRevert("Ownable: caller is not the owner");
