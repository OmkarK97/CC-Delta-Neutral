...
import "./OracleInterface/IChainLinkPrice.sol";
    ...
    ReentrancyGuard //PdnRiveraVolatileBase
{
    ...
    address public pId; // id of a token to get its price from oracle
    address public pIdB; // id of a tokenB to get its price from oracle
  ...
  */
    ...
    function tokenToTokenConversion(
        ...
        uint256 tokenAPrice = uint256(
            int256(chainLinkPrice(idA).latestAnswer())
        );

        uint256 tokenCPrice = uint256(
            int256(chainLinkPrice(idB).latestAnswer())
        );
    ...
    function getTokenPrices() external returns(uint,uint){
        basePrice = uint256(
            int256(chainLinkPrice(pId).latestAnswer())
        );

        tokenBPrice =uint256(
            int256(chainLinkPrice(pIdB).latestAnswer())
        );

        return(basePrice , tokenBPrice);
    ...
    function balanceOf() public view returns (uint256) {
        return balanceRivera() + balanceDeposit() - totalDebt() + IERC20(baseToken).balanceOf(address(this));
    }
